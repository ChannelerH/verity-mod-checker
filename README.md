...
- Old Java 1.0.0 route: answers `verity-1.0.0.jar`, `Verity Mod 1.0.0`, Google Drive, Discord, MediaFire, Mega, Forge 1.20.1, and Geckolib mirror claims; it records the old filename as a legacy or mirror claim and routes players back to the current Verity JE 5.7.3 record instead of hosting files.
...
The checker verifies identity signals against the public metadata recorded on July 30, 2026. The reusable snapshot is published at [`/data/verity-releases.json`](https://veritymodchecker.online/data/verity-releases.json), with route updates in [`/feed.xml`](https://veritymodchecker.online/feed.xml). The daily health job fails when that snapshot is more than 72 hours old or when the homepage, checker, download page, Java guide, and feed disagree about the current release map. Stable Verity JE 5.7.3 is checked on CurseForge (`1591438` / `8461257`) and Modrinth (`on1Y0osD` / `yAt0wv1Z`); the observed Verity JE 6.0.1 and 6.0.0 beta records (`YLEoXe6t`, `5ech0sTo`) now return 404 and are treated as stale-link status records. Verity BE 1.1.0 is checked on CurseForge (`1574632` / `8506198`) with 4,048,008 project downloads at the July 30 source check; PnTMC Verity 3.2.0 is checked on CurseForge (`1575941` / `8517480`, `8517478`, `8517473`) with 7,383,790 project downloads, with old 2.1.0 records (`8327253`, `8327250`, `8327242`) separated from current downloads. The July 30 modpack snapshot separates CurseForge `VERITY.exe` (`1585389`) from Modrinth `VERITY.exe Remastered` (`L5qUPsXS` / `4N4vQrK7`) and now also maps monster-form/profile routes such as `Verity May Be` (`1583253` / `8365755`), `VerityCraft` (`1600389`), and `Verity World` (`1598871` / `8473476`). The browser checker can compare a local Java JAR with Modrinth's published SHA-512 without uploading it. The similar-name snapshot separates Verity Dweller (`eZW2ZX0U`), Horrorland with Verity (`mJcQB7OR`), a generic Verity Paper plugin (`DYiZP3fM`), Verity Body Overhaul (`1629639` / `8530009`), and veritycf (`1600657` / `8503252`) from the current Verity Mod routes. A filename, size, URL, or hash result is not a malware verdict. Downloads remain on maintainer-controlled records instead of being mirrored by this site.

...
- Smiley's Better Voice current CurseForge route: Project ID 1608717, owner Toast5, 101,848 project downloads on the official files/all page at check, current file `Smiley's Better Voice-4.0.0.jar`, record 8536538, 3.3K file-list downloads, 1.4 MB. The exact file page rendered 101,479 project downloads and 3.0K file downloads during the same check, so treat these as point-in-time display counters.
...
- Official older Better Voice rows visible at check include 3.1.1, 3.1.0, 3.0.0, multiple 2.0.x Forge rows for 1.20.1, and a separate 2.0.0 NeoForge row for 1.21.1. Use the official files/all page and exact file pages before pairing an older add-on with an older parent Verity JE route.
...
4. If the parent route is intentionally older, open the official Better Voice files/all page and exact older file page before choosing a 3.x or 2.x row.
5. For VERITY.exe, test direct chat first, then separate voice chat, Groq key, provider quota, TTS, microphone permission, and profile setup.
6. For Java Verity JE, prove one short typed prompt before debugging live microphone input or Simple Voice Chat.
7. For Bedrock Verity BE, activate behavior and resource packs together, enable the required world options, and use `/verity:tts on` when that route documents local TTS.
8. For Android Java launchers, verify microphone permission and inspect the independent Smiley OpenAL route separately from the Verity JE file.
9. If a provider status code appears, use the API page instead of a voice-only fix: https://veritymodchecker.online/api-connection-failed/
